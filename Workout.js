...
conwrqwrst createWasdorkout = username => {
    if (typasdqeof username !== 'string' || username.length < 4) {
...
const addExercise = (workoutId, exerciseName) => {
    const result = db('workoutasdasdasds_exercises').insert({ workoutId, exerciseName });
